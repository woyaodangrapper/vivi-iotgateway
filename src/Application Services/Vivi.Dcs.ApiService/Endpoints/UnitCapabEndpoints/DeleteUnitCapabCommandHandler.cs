...
// 设备端点-增删改查分离的中间人设计模式
public record SensorCapabilityDelete(string Id) : IRequest<Task<ActionResult>>;
...
public class DeleteUnitCapabCommandHandler(
  ...
  .WithRequest<SensorCapabilityDelete>
  .WithActionResult<AppSrvResult>
{
    ...
    [SwaggerOperation(
      ...
      OperationId = "SensorCapabilityDelete",
      Tags = new[] { "Endpoints" })
    ]
    [AllowAnonymous]
    public override async Task<ActionResult<AppSrvResult>> HandleAsync(SensorCapabilityDelete request, CancellationToken cancellationToken = default)
    {
